...
monthsBetween (Date dA) (Date dB) =
    case (dA.month, dB.month) of
        (Just mA, Just mB) ->
            let
                totalMonths =
                    abs ((dA.year - dB.year) * 12 + monthToInt mA - monthToInt mB)
            in
            Just totalMonths
...
compare (Date d1) (Date d2) =
    case (Basics.compare d1.year d2.year, d1.month, d2.month) of
        (EQ, Just m1, Just m2) ->
...
view (Date d) =
    let
        ...
        monthHtml =
            case d.month of
                Just m ->
                    div [] [ text <| monthToString m ]

                Nothing ->
                    div [] []
    in
    div [] [ yearHtml, monthHtml ]
...
compareMonth m1 m2 =
    compare (Date { year = 0, month = Just m1 }) (Date { year = 0, month = Just m2 })
